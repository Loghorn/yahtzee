...
{
	return accumulate(begin(scores), end(scores), static_cast<unsigned short>(0), [](unsigned short val, const Score& score) { return val + score.value; });
}
...
void ScoreTable::AssignScoreIfNotAssigned(Scores::ScoreName scoreName, unsigned short score, bool markAsAssigned)
{
...
{
	for (auto i = 0u; i < table.scores.size(); ++i)
